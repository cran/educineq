...
#'
#' @inheritParams emean
#' @return \code{etheil} returns a list with the following objects:
#'   \enumerate{
...
#' @details The estimates of the Theil index for the specified group of countries
...
etheil <- function(countries, init.y, final.y, database, plot  = TRUE) {
  ...
  for(k in 1:length(time)){
    ...
    i.x<-ok.data$theil[ok.data$year==time[k]]
    w <- ok.data$pop[ok.data$year==time[k]]/sum(ok.data$pop[ok.data$year==time[k]])
    m <- sum(w*x)
    si<-w*x/m
    theil[1, k] <- sum((w*x/m)*log(x/m))
    theil[2, k] <- sum(si*i.x)
  ...
  if (plot==TRUE){
    ...
    legendtext <- c("Theil index", "Between-country", "Within-country")
    legend("top", legend = legendtext, cex = 0.8, ncol = 3,
    lty = 1, col = c("Blue", "Red", "green4"))
  }
  colnames(theil) <- time
  row.names(theil) <- c("Between", "Within", "Theil index")
  list(Theil_index = theil, countries = unique(ok.data$country))
}
